  ...
  const onCreateTransaction = async (data: TransactionForm) => {
    ...
    try {
      ...
      try {
        ...
        recipient = userResponse.data.user;
      } catch (error: any) {
        if (error.response?.status === 404) {
          toast.error(`User with email ${data.recipientEmail} not found`);
        } else {
      ...
      await transactionApi.create(Number(data.amount), recipient._id, data.description);

...
import { 
  Plus, 
  BookUser, // Changed from Handshake to BookUser
  TrendingUp,
  TrendingDown,
  CheckCircle,
  Clock,
  ...
  const onCreateLoan = async (data: LoanForm) => {
    ...
    try {
      ...
      try {
        ...
        borrower = userResponse.data.user;
      } catch (error: any) {
        if (error.response?.status === 404) {
          toast.error(`User with email ${data.borrowerEmail} not found`);
        } else {
      ...
      // Cannot create a loan to yourself
  ...
  return (
    <div className="space-y-6">
      ...
      {filteredLoans.length === 0 ? (
        <EmptyState
          icon={<BookUser className="w-12 h-12" />} // Changed from Handshake to BookUser

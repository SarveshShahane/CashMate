  ...
  const onCreateExpense = async (data: CreateExpenseForm) => {
    ...
    try {
      await expenseApi.create(
        groupId!, 
        Number(data.amount), // Ensure amount is converted to number
        data.description, 
        selectedMembers
      );
        ...
        <form onSubmit={handleExpenseSubmit(onCreateExpense)} className="space-y-4">
          ...
          <div>
            ...
            <input
              {...registerExpense('amount', {
                ...
                validate: value => !isNaN(Number(value)) || 'Amount must be a valid number'
              })}
              type="number"
              step="0.01"
              className="input"
          ...
          <div>
            ...
            {selectedMembers.length === 0 && (
              <p className="mt-1 text-sm text-danger-600">Please select at least one member</p>
            )}
          </div>
